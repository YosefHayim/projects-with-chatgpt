from Binance_telegram_RSI_SHORT import send_telegram_message, format_order_message
import logging
import talib
import numpy as np
import time
from Binance_setup_DGBUSDT_RSI_SHORT import BinanceBot
from Binance_config_DGBUSDT_RSI_SHORT import api_key, api_secret
    ...
    while True:
        try:
            ...
            if real_time_data and len(real_time_data) > 0:
                ...
                if current_rsi > RSI_UPPER_BOUND and not in_short_position and total_active < MAX_ACTIVE_POSITIONS:
                    order = bot.place_market_order('SELL', quantity)
                    log_order(order)
                    logging.info("Entering Short position")
                    in_short_position = True

                # Exit Short Position
                elif current_rsi < RSI_LOWER_BOUND and in_short_position:
                    order = bot.clean_open_orders()
                    log_order(order)
                    logging.info("Exiting Short position")
                    in_short_position = False
